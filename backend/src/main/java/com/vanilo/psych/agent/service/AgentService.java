...
public class AgentService {
    ...
    public AgentService(ChatClient.Builder clientBuilder,
...
get_dashboard 的 username 会由后端根据登录态自动注入，模型不需要生成 username。
    ...
    private String generateFinalReply(String message,Object toolResult,String memoryContext){
...
如果工具结果是 dashboard 数据，请概括最近报告和高风险统计情况。
                ...
                .user(
                        """
                                ...
                                工具结果：
                                %s
                                """.formatted(message,memoryContext,toolResultJson)
                )
                .call()
                .content();
    }
    private String generatePlainReply(String message,String memoryContext){
        return chatClient.prompt()
                .system("""
                ...
                .user("""
                        ...
                        用户消息：
                        %s
                        """.formatted(memoryContext, message))
                .call()
                .content();
    }
}
